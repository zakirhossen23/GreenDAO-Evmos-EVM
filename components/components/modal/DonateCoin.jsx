...
export default function DonateCoin({ ideasid, show, onHide, address }) {
	...
	async function DonateCoinSubmission(e) {
		...
		let output = await sendTransfer(contract, sendTransaction,Number(ideasid), amount.value, ShowAlert);
		// setTransaction({
		// 	link: output.transaction
		// });
		// Saving Donation count on smart contract
		await sendTransaction(contract.add_donation(Number(ideasid), (Number(amount.value) * 1e18).toFixed(0)));


		ShowAlert("success", `Successfully donated ${amount.value} tEVMOS! `);
	...
	async function LoadData() {
		...
		setBalance((Balance / 1e18).toFixed(5) + " tEVMOS");
		setCurrentAddress(window.ethereum.selectedAddress.toString());
	}
	useEffect(() => {
		LoadData();
	}, [show]);
